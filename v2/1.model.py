...
def replaceWord(TEXT):
    REP_SPECIAL = re.compile(r'[@.:/\&;=%$#?+{}()[\],|!\*\'\"_–·]')
    TEXT = REP_SPECIAL.sub('', TEXT)
    # TEXT = TEXT.replace(' ', '')
    return TEXT
...
def buildDataset(WORDS, VOCABULARY_SIZE):
    ...
    VOCABULARY_TOP_COUNT.extend(collections.Counter(WORDS).most_common(VOCABULARY_SIZE - 1))
    DATASET = dict()
...
def generateBatch(BATCH_SIZE, SKIPS_NUM, SKIP_WINDOW, DATA, DATA_INDEX):
...
def buildModel(WORD_TOKEN_ALLTHREAD):
    VOCABULARY_SIZE = 100000
    DATA, VOCABULARY_TOP_COUNT, DATASET, REVERSE_DATASET = buildDataset(WORD_TOKEN_ALLTHREAD, VOCABULARY_SIZE)
    # print(DATA, VOCABULARY_TOP_COUNT, DATASET, REVERSE_DATASET)
    createFile({"data":DATA, "vocabulary_top_count":VOCABULARY_TOP_COUNT, "dataset":DATASET, "reverse_dataset": REVERSE_DATASET}, "dataset")
